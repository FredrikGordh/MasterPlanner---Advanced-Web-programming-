...
router.post("/My_profile", async (req, res) => {
  ...
  if (database.length > 0) {
    insertQuery =
      "UPDATE userInfo set (Name, ProfileEmail, LiuID, Master) = (?,?,?,?) where (Owner) = (?)";
  } else {
    insertQuery =
      "INSERT or IGNORE INTO userInfo(Name, ProfileEmail, LiuID, Master, Owner) VALUES (?,?,?,?,?)";
  }
...
router.post("/Update_image", async (req, res) => {
  ...
  if (database.length > 0) {
    ...
    insertQuery =
      "INSERT or IGNORE INTO userInfo(Name, ProfileEmail, LiuID, Master, imgURL, Owner) VALUES (?,?,?,?,?,?)";
  }
