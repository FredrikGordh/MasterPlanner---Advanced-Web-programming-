...
router.delete("/MyCourses", async (req, res) => {
  let deleteQuery =
    "DELETE FROM MyCourses WHERE (Kurskod, Kursnamn, HP, Nivå, Block, VOF, Säsong, Period) = (?,?,?,?,?,?,?,?)";
  const myCourses = [
    req.body.Kurskod,
    req.body.Kursnamn,
    req.body.HP,
...
router.post("/MyCourses/:owner", async (req, res) => {
  ...
  if (req.body.Master === undefined) {
    insertQuery =
      "INSERT or IGNORE INTO MyCourses(typ, Kurskod, Kursnamn, HP, Nivå, Block, VOF, Säsong, Period, Owner) VALUES (?,?,?,?,?,?,?,?,?,?)";
    const insert = await db.runAsync(insertQuery, myCourses);
    database = await db.allAsync(sql);
  } else {
    insertQuery =
      "UPDATE MyCourses set Master = (?) where (typ, Kurskod, Kursnamn, HP, Nivå, Block, VOF, Säsong, Period, Owner) = (?,?,?,?,?,?,?,?,?,?)";
    const update = await db.runAsync(insertQuery, [
      req.body.Master,
      myCourses[0],
      myCourses[1],
      myCourses[2],
